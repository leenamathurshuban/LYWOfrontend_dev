...
const CreateJobs = ({ show, handleClose }) => {
  ...
  const [isLike, setIsLike] = useState("");
  const [isLikeUid, setIsLikeUid] = useState([]);
  // FIX #6: track selected isLike label for display
  const [isLikeLabel, setIsLikeLabel] = useState("");
  const [isLikeDropdown, setIsLikeDropdown] = useState(false);
  const [isLikeData, setIsLikeData] = useState([]);
  const [isDisabledTarget, setIsDisabledTarget] = useState(true);
  const [ishideIndustries, setIsHideIndustries] = useState(true);
  ...
  const [behaviours, setBehaviours] = useState([
    ...
    },
    {
      heading: "Thoughtfulness",
      data: "Thinking carefully aboutthings before acting",
      uid: "52261636-2fab-4935-ad9c-f2a3fa9f8be4", // FIX: removed leading spaces in uid
      markedImportant: false,
      isSelected: false,
    },
    {
  ...
  const [updateFormData, setUpdateFormData] = useState({
    ...
    currency: "INR",
    salary_type: "",
    // FIX #4: unified hyphen to match reset value
    salary_price_type: "Salary-range",
    workplace_type: "",
    job_type: "",
    description_attachment: "",
  ...
  const MAX_FILE_SIZE = 5 * 1024 * 1024;
  // FIX #2: renamed for clarity — this is a word count minimum
  ...
  // FIX #2: Corrected word count validation logic
  const getWordCount = (htmlStr) => {
  ...
  const CheckValidation = () => {
    ...
    if (!isLikeUid.length) {
      newErrors.isLike = "Is Like is required";
      isValid = false;
    }
    ...
    // FIX #2: proper word count check
    if (!description) {
      newErrors.description = "This Field is required";
      isValid = false;
    } else if (getWordCount(description) < MIN_DESCRIPTION_WORDS) {
      newErrors.description = `Minimum ${MIN_DESCRIPTION_WORDS} words required`;
      isValid = false;
    }
    ...
    if (!createFormData.noOfPosition) {
      newErrors.noOfPosition = "No. of Positions is required";
      isValid = false;
    }
  ...
  const handleFormData = (e) => {
    ...
    const { isErrors, isValid } = CreateJobFormValidation(newValue);
    setErrors((prev) => ({ ...prev, ...isErrors }));
    setCreateFormData((prev) => ({ ...prev, ...newValue }));
  };

  // FIX #3: unified handler for both Job Type and Workplace Type selects
  const handleJobType = (val) => {
    setCreateFormData((prev) => ({ ...prev, jobType: val?.value || "" }));
    setErrors((prev) => ({ ...prev, jobType: "" }));
  };

  const handleWorkPlaceType = (val) => {
    setCreateFormData((prev) => ({ ...prev, workPlaceType: val?.value || "" }));
    setErrors((prev) => ({ ...prev, workPlaceType: "" }));
  };
  ...
  // FIX #6: store label for display
  const handleSelectedLikeItems = (item) => {
    setIsLikeUid([item.value]);
    setIsLikeLabel(item.label);
    setErrors((prev) => ({ ...prev, isLike: "" }));
  };

  // FIX #1: Corrected editor change logic (was inverted)
  const handleEditorChange = (value) => {
    ...
    if (wordCount >= MIN_DESCRIPTION_WORDS) {
      ...
      setDescriptionError(`Minimum ${MIN_DESCRIPTION_WORDS} words required. Current: ${wordCount} words.`);
      // Only clear the submit error once they start typing
      if (wordCount > 0) {
        setErrors((prev) => ({ ...prev, description: "" }));
      }
    } else {
  ...
  const handleFileUpload = (e) => {
    ...
    if (file.size > MAX_FILE_SIZE) {
      setErrorMessage("Attachment failed. The attachment exceeds the allowed file size.");
      return;
    } else {
      setErrorMessage("");
    }
  ...
  const handleTravelChange = (e) => {
    setTravelOption(e.target.value);
    setErrors((prev) => ({ ...prev, travelOption: "" }));
  };
  ...
  const isLikeHandleApi = (query) => {
    ...
    CreateJobIsLike(url)
      .then((res) => {
        const key = res?.data?.response?.map((val) => ({
          value: val?.uid,
          label: val?.is_like_name,
      ...
      .catch((error) => {
        if (
          error?.response?.status === 401 ||
          error?.response?.data?.detail?.includes("Given token not valid for any token type")
        ) {
          removeToken();
          navigate("/loginwithpassword");
        }
  ...
  const benifitsList = () => {
    GetBenifints()
      .then((res) => setBenefitsData(res.data.response))
      .catch((error) => {
        if (
          error?.response?.status === 401 ||
          error?.response?.data?.detail?.includes("Given token not valid for any token type")
  ...
  const hasSelectedAndImportant = behaviours.some(
    (item) => item?.isSelected || item?.markedImportant
  );
  ...
  // FIX #8: clear location error on selection
  const handleLocationItems = (item) => {
    setLocationUid(item.value);
    setIsLocationDropdown(false);
  ...
  const handleCreateForm = async () => {
    if (CheckValidation()) {
      ...
      try {
        const response = await CreateJobForm(formdata);
        if (response.data.status === 200) {
          const newUid = response.data.response.uid;
          setCreateJobUid(newUid);
          // FIX #10: also set createUid so it is correctly passed to the child
        ...
        if (error?.response?.data?.response) {
          Object.entries(error.response.data.response).forEach(([key, err]) => {
            setErrors((prev) => ({ ...prev, [key]: err[0] }));
          });
        }
        if (
          error?.response?.status === 401 ||
  ...
  const handleUpdateForm = async () => {
    ...
    for (const key in updateFormData) {
      if (key === "area_of_education" && badges.length > 0) {
        let uids = badges.map((item) => item?.uid);
        formdata.append(key, JSON.stringify(uids));
      } else if (key === "shortlisted_industry" && IndustriesBadges.length > 0) {
        let uids = IndustriesBadges.map((item) => item?.uid);
        formdata.append(key, JSON.stringify(uids));
      } else if (key === "restricted_roles" && restrictedRoleBadges.length > 0) {
        let uids = restrictedRoleBadges.map((item) => item?.uid);
        formdata.append(key, JSON.stringify(uids));
      } else if (key === "spoken_language" && spokenLanguageBadges.length > 0) {
        let uids = spokenLanguageBadges.map((item) => item?.uid);
        ...
        let skillID = SelectSkillsData?.map((item) => item?.uid)?.filter((skill) => skill !== "");
        formdata.append("skills", JSON.stringify(skillID));
      } else if (key === "must_have_skills" && mustHaveSkills.length > 0) {
        let skillID = mustHaveSkills?.map((item) => item?.uid);
        formdata.append("must_have_skills", JSON.stringify(skillID));
      } else if ((key === "selected_behaviour" || key === "important_behaviour") && hasSelectedAndImportant) {
        let selectedBehaviourUids = [];
        let importantBehaviourUids = [];
        ...
        behaviours.forEach((item) => {
          if (item?.isSelected) selectedBehaviourUids.push(item.uid);
          if (item?.markedImportant) importantBehaviourUids.push(item.uid);
        });

        formdata.append("selected_behaviour", JSON.stringify(selectedBehaviourUids));
        formdata.append("important_behaviour", JSON.stringify(importantBehaviourUids));
      } else if (
        key === "higher_qualification_preferred" ||
        key === "other_areas_acceptable" ||
        key === "non_negotiable_salary" ||
        ...
        formdata.append(key, ishideIndustries ? "True" : "False");
      } else if (key === "define_current_role") {
        formdata.append(key, isHideRestrictedRoles ? "True" : "False");
      } else if (key === "immediate_hiring") {
        formdata.append(key, updateFormData.targate_hire_date !== "" ? "False" : "False");
      } else if (key === "no_specific_language_require") {
        formdata.append(key, isSpecificLanguareRequired ? "True" : "False");
      } else if (key === "no_specific_location") {
        formdata.append(key, isHideLocation ? "True" : "False");
      } else if (key === "is_salary_imp" && importantFlag.salary) {
        formdata.append("is_salary_imp", importantFlag.salary);
      } else if (key === "is_education_imp" && importantFlag.education) {
        formdata.append("is_education_imp", importantFlag.education);
  ...
  const isNextButtonDisable =
    ...
    createFormData.jobType &&
    createFormData.noOfPosition &&
    createFormData.workPlaceType &&
    isLikeUid.length &&
    locationUid;
  ...
  return (
    ...
    >
      ...
      <Offcanvas.Body>
        {/* FIX #5: autocomplete -> autoComplete (valid JSX prop) */}
        <Form className="row" onSubmit={(e) => { e.preventDefault(); handleCreateForm(); }} autoComplete="off">
          <Form.Group className="col-md-12 mb-2" controlId="jobTitle">
            <Form.Label>Job Title</Form.Label>
            <Form.Control
              type="text"
          ...
          <Form.Group className="col-md-6 mb-2 relative" controlId="isLike">
            ...
            <Select
              ...
              styles={customStyles}
            />
            {/* FIX #7: show isLike error */}
            {errors.isLike && (
              <span className="error-msg" style={{ color: "red", fontSize: 12 }}>
                {errors.isLike}
              </span>
            )}
          </Form.Group>
          ...
          <Form.Group className="mb-2" controlId="jobDescription">
            ...
            <div className="texteditor_warp" onClick={handleWrapperClick}>
              ...
              />
              <div className="custome_file" style={{ display: "flex", alignItems: "center" }}>
                <label
                  htmlFor="file-upload"
                  style={{
                    cursor: "pointer",
                ...
                <div>
                  <p
                    className="m-0"
                    style={{ fontSize: 12, color: errorMessage ? "red" : "black" }}
                  >
                    {errorMessage
                      ? errorMessage
                      : fileName
                ...
                {uploadProgress > 0 && uploadProgress < 100 && (
                  <>
                    <div className="progress-bar" style={{ height: "10px", width: "500px", backgroundColor: "white" }}>
                      <div
                        className="progress"
                        style={{ width: `${uploadProgress}%`, backgroundColor: "blue" }}
                      ></div>
                    </div>
                    <span>{uploadProgress}%</span>
                  </>
            ...
            {/* FIX #1: show live word count feedback */}
            {descriptionError && <div className="error" style={{ color: "orange", fontSize: 12 }}>{descriptionError}</div>}
            {errors.description && <div className="error" style={{ color: "red", fontSize: 12 }}>{errors.description}</div>}
            {errors.detailed_description && (
              <div className="error" style={{ color: "red", fontSize: 12 }}>{errors.detailed_description}</div>
            )}
          </Form.Group>
          ...
          <Col md={6} className="mb-2">
            <Form.Label>Job Type</Form.Label>
            <div className="mw-230 relative">
              {/* FIX #3: use dedicated handleJobType handler */}
              <Select
                className="react_selectbox"
                ...
                value={jobTypeOptions.find((o) => o.value === createFormData.jobType) || null}
              />
            </div>
            {errors.jobType && (
              <span className="error-msg" style={{ color: "red", fontSize: 12 }}>
          ...
          <Col md={6} className="mb-2">
            <Form.Label>Workplace Type</Form.Label>
            <div className="mw-230 relative">
              {/* FIX #3: use dedicated handleWorkPlaceType handler */}
              <Select
                styles={customStyles}
                className="react_selectbox"
                ...
                value={workplaceOption.find((o) => o.value === createFormData.workPlaceType) || null}
              />
            </div>
          ...
          <Form.Group className="mb-2" controlId="benefits">
            ...
            <div className="tagarea">
              ...
              {addCustomeBenifits.map((item, index) => (
                <div key={index} className="input-container position-relative">
                  <input
                    value={customValue}
                    onChange={(e) => setCustomValue(e.target.value)}
                    className={`badge-gray ${SelectBenefitsData.includes(item) ? "active" : ""}`}
