...
//   const getQualificationListByCourse = async (e, inputVal) => {
//     let url;
...
//       if (inputVal) {
//         setBadges((prevBadges) => [...prevBadges, response?.data?.response[0]]);
...
//                 <ul>
//                   <li>
//                   <span className="text-danger italic">{!!createRevisedJobData&&(createRevisedJobData?.skills.forEach((item)=>item.skill_name)?.join(","))}</span>
//                   </li>
//                 </ul>
//               </div>
//               <div className="user_bsinfo">
...
// <li>Area: {createRevisedJobData?.job_location?.location_name}</li>

// <li>Experience: {createRevisedJobData?.min_exp}-{createRevisedJobData?.max_exp} Years</li>
...
import Edit03 from "../../images/icons/edit-0303.svg";
import logoIcon from "../../images/logo_icon.png";
import { getJobDetailsApi,getQualificationListApi } from "../../services/provider";
import { removeToken } from "../../helpers/helper";
import { useNavigate } from "react-router-dom";

const CreateJobsRevised = ({ show, handleClose,handleCreateForm,handleFormData,setCreatedUid,setBadges,badges,minEdu,setMinEdu,setIsUpdated,isUpdated }) => {
  const [createRevisedJobData, setCreateRevisedJobData] = useState(null);
      ...
      <Modal.Header closeButton>
        ...
        {/* {createRevisedJobData.map((item) => ( */}
          <Modal.Title>
            ...
            <span className="subtitle">
              {createRevisedJobData?.job_location?.location_name},{" "}
              {createRevisedJobData?.department?.department_name}, {createRevisedJobData?.job_type},{" "}
              {createRevisedJobData?.workplace_type}
            </span>
            <button type="button" className="edit-btnicon">
              <img src={Edit03} />
      ...
      <Modal.Body className="p-0 bg-lightgray">
        <Row className="justify-content-center">
                ...
                <Accordion.Body>
                  <Row className="align-items-center mb-3">
                    <div className="form-group w-auto mb-0">
                      ...
                      name="salary_price_type"
                        aria-label="Default select example"
                        onChange={(e)=>{handleFormData(e);setPriceRangeType(e.target.value);console.log(e.target.value)}}
                        className="sm-fselect"
                      >
                        <option selected value="Salary-range" >Salary Range</option>
                        <option value="Min-salary" >Min. Salary</option>
                        <option value="Max-salary" >Max. Salary</option>
                      </Form.Select>
                    </div>
                   ...
                   {priceRangeType==="Salary-range"&&<div className="form-group  w-auto mb-0 px-0">
                    ...
                    {(priceRangeType === "Salary-range" || priceRangeType === "Min-salary") && (
                      <div className="form-group w-150 mb-0">
                        <Form.Control
                          type="text"
                          name="min_salary"
                    ...
                    {(priceRangeType === "Salary-range" || priceRangeType === "Max-salary") && (
                      <div className="form-group w-150 mb-0">
                        <Form.Control
                          type="text"
                          name="max_salary"
                  ...
                  <div className="accordion_footer">
                    ...
                    <button onClick={handleCreateForm} type="button" class="btn btn-lightgray">
                      Next
                    </button>
                  </div>
                </Accordion.Body>
              ...
              <Accordion.Item eventKey="2" className="accd_child" id="item_edu">
                ...
                <Accordion.Body>
                  <Form>
                    <Form.Group className="mb-3">
                      ...
                      <Form.Select
                        ...
                        className="sm-fselect"
                        name="minimum_education"
                        onChange={(e)=>{setMinEdu(e.target.value);handleFormData(e)}}
                      >
                        <option>Select...</option>
                        <option value="high_school">High school</option>
                      ...
                      {badges.map((badge, index) => (
                        <Badge key={index} bg="light" className="me-2 mb-2">
                          ...
                          <button className="btn" style={{ cursor: "pointer" }}
                            onClick={() => handleRemoveBadge(index)}><i className="fa fa-close ms-1"></i></button>
                        </Badge>
                      ))}
                      <Form.Control
                      type="text"
                  ...
                  <div className="accordion_footer">
                    ...
                    <button type="button" onClick={handleCreateForm} class="btn btn-lightgray">
                      Next
                    </button>
                  </div>
                </Accordion.Body>
          ...
          {console.log(createRevisedJobData,"CreateiveeavisedJob")}
            <h5>{createRevisedJobData?.job_title}</h5>
            <p>
              Like {createRevisedJobData?.department?.department_name}, {createRevisedJobData?.createRevisedJobData?.job_title} Positions
              <br />
              Technology, {createRevisedJobData?.job_location?.location_name},
              <br />
              {createRevisedJobData?.job_type}, {createRevisedJobData?.workplace_type}
            </p>
            <div className="ct_scrollbar pb-3">
              <div className="user_bsinfo">
                <h6>Expectations</h6>
                <ul>
                  <li>{createRevisedJobData?.min_exp}-{createRevisedJobData?.max_exp} Years Experience </li>
                  <li>
                    Salary{" "}
                    <span className="text-danger italic">{createRevisedJobData?.max_salary}</span>{" "}
                  </li>
                  <li>
                    Minimum Qualification{" "}
                    <span className="text-danger italic">Not defined</span>{" "}
                  ...
                  <li>
                    Education{" "}
                    <span className="text-danger italic">{createRevisedJobData?.minimum_education}</span>{" "}
                  </li>
                </ul>
              </div>
              <div className="user_bsinfo">
