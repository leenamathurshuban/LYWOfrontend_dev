...
const CompanyEditProfile = ({ show, handleClose }) => {
  ...
  const zoomIn = () => {
    if (zoom < 3) {
      ...
      setLogoModal(false);
      setIsZoomedImg(false);
      alert("You cannot zoom in further. Maximum zoom level reached.");
    }
  };
  ...
  const zoomOut = () => {
    if (zoom > 1) {
      ...
      setLogoModal(false);
      setIsZoomedImg(false);
      alert("You cannot zoom out further. Minimum zoom level reached.");
    }
  };
  ...
  const handleSave = () => {
    const img = new Image();
    img.crossOrigin = "anonymous"; // This allows CORS requests
    img.src = image;
    ...
    return (
      <div className="ctm_modal_dialog">
        <div className="ctm_modal_content">
          ...
          <Modal.Footer>
            <Button onClick={handleLogoCancel} variant="light">
              Cancel
            </Button>
            <Button onClick={handleSave} variant="primary">
              Save
                              ...
                              <div className="cmp_logo mt-1">
                                <span className="cmp_textlogo">
                                  <img
                                    src={
                                      croppedImage
                                      //|| `https://bittrend.shubansoftware.com${companyProfileDetails?.logo}`
                                    }
                                    alt="Logo"
                                  />
                                </span>
